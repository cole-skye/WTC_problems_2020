import robot, sys, turtle, import_helper
...
def update_position(robot_name, steps):
    ...
    if maze.is_path_blocked(position_x, position_y, new_x, new_y):
        obstacles_flag = True
        return False
...
def screen_size():
    ...
    screen.setworldcoordinates(-100, -200, 100, 200)
...
def show_obstacles():
    ...
    """
    global maze_obstacles

    maze.get_obstacles()
    for ob in maze.obstacles_list:
        t.color("white")
        t.goto(ob)
        t.pd()
        t.begin_fill()
    ...
    maze_obstacles = maze.obstacles_list 


def maze_run_turtle(edge):
    """
    Gets called from the robot file and passes a parameter "edge'
    which tells the maze solver which edge to go to.
    """

    return mazerunner.maze_run(edge)
