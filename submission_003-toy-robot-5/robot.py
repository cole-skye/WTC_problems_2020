...
import sys
if 'turtle' in argv:
    from world.turtle import world as wt
else: 
    from world.text import world as wt
...
def valid_command(command):
    ...
    if command_name.lower() == 'replay':
        ...
        return (command_name.lower() in valid_commands and (len(arg1) == 0 or is_int(arg1)) )or maze_check(command)


def maze_check(command):
    """
    Validation check to make sure a valid argument is passed with the command 'mazerun'.
    """

    maze_valid = ['bottom', 'left', 'right', 'top']

    command.lower()
    if 'mazerun' in command:
        if command.split()[0] == 'mazerun':
            if command.split()[1] not in maze_valid:
                return False
            else:
                return True
    else:
...
MAZERUN - solves maze [top, bottom, left, right]
...
def do_sprint(robot_name, steps):
    ...
    if steps == 1:
        return wt.do_forward(robot_name, 1)
    else:
        (do_next, command_output) = wt.do_forward(robot_name, steps)
...
def call_command(command_name, command_arg, robot_name):
    if command_name == 'help':
        ...
        return wt.do_forward(robot_name, int(command_arg))
    elif command_name == 'back':
        return wt.do_back(robot_name, int(command_arg))
    elif command_name == 'right':
        return wt.do_right_turn(robot_name)
    elif command_name == 'left':
        return wt.do_left_turn(robot_name)
    elif command_name == 'sprint':
        return do_sprint(robot_name, int(command_arg))
    elif command_name == 'replay':
        return do_replay(robot_name, command_arg)
    elif command_name == 'mazerun':
        return do_maze_run_robot(robot_name, command_arg)
    return False, None


def do_maze_run_robot(robot_name, edge):
    """
    Handles the 'mazerun' command, also if no edge is defined it calls the 'top'
    edge by default.
    calls the maze_run function that is located in turtle., which calls a mazerun function in 'mazerunner'.
    the function return a set of commands which is passed through handle_command.
    """
    ...
    if edge == '':
        edge = 'top'
    command_list = wt.maze_run_turtle(edge[0].upper())
    ...
    if edge[0].upper() == 'T':
        return True, robot_name+f": I am at the top edge."
...
def robot_start():
    ...
    output(robot_name, f"Loaded {sys.argv[-1]}.")

    if 'turtle' not in argv:
        wt.obs_aware()
